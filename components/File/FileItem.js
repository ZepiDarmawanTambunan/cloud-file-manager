  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 justify-between cursor-pointer hover:bg-gray-100 p-3 rounded-md'>
      <div className='flex gap-2 items-center'>
        <Image src="/pdf.png" alt='file-icon' width={26} height={20} on/>
        <h2 className='text-[15px] truncate' onClick={()=>window.open(file.imageUrl)}>
          {file.name}
        </h2>
      </div>
      <div className='grid grid-cols-3 place-content-start'>
          ...
          <h2 className='text-[15px]'>
            {(file.size.split(' ')[0]/1024 ** 2).toFixed(2)+" MB"}
